...
describe("turn diagnostics flag", () => {
  ...
  test("accepts the same truthy spellings as the information-layers flag", () => {
...
describe("turn diagnostics output", () => {
  test("records the flags, escalation tier and scenario for the turn", () => {
    withFlag("true", () => {
      ...
      assert.match(out, /informationLayers=(true|false)/);
    });
  });
  ...
  test("reports the alignment gate even when the flag leaves it inactive", () => {
    withFlag("true", () => {
      const out = capture(() => logTurnInputs(CTX));
      assert.match(out, /alignmentGate: (ACTIVE|derived-but-INACTIVE\(flag off\))/);
      assert.match(out, /useCase=(true|false)/);
    });
  });
