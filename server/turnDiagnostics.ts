...
import { deriveConversationState, deriveDirectQuestion } from "./conversationState";
import { buildTurnStateBlock } from "./llm";
...
// spellings the application accepts for feature flags, so operators only learn
// one convention.
...
export function logTurnInputs(ctx: TurnDiagnosticsContext): void {
  ...
  try {
    ...
    const parts = [
      ...
      `  context: escalationTier=${ctx.escalationTier}`,
      ...
      `  ${fence("  stateBlockAppendedToPrompt", buildTurnStateBlock(ctx.history) || "(empty)")}`,
    ];
