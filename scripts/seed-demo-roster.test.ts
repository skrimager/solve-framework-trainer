...
describe("demo roster transcript and coaching backfill", () => {
  test("includes the six Demo Office personas and four existing Apptix usernames without creating substitute accounts", () => {
    assert.equal(DEMO_OFFICE_PERSONAS.length, 6);
    assert.deepEqual(APPTIX_PERSONAS.map((persona) => persona.username), ["demo_morgan", "demo_taylor", "demo_avery", "demo_riley"]);
    assert.equal(APPTIX_PERSONAS.reduce((count, persona) => count + persona.sessions.length, 0), 36);
    assert.equal(
      [...DEMO_OFFICE_PERSONAS, ...APPTIX_PERSONAS].reduce((count, persona) => count + persona.sessions.length, 0),
      90,
    );
  });
  ...
  test("creates coaching rows with schema-required fields, allowed roles, and post-session timestamps", () => {
    ...
    for (const message of messages) {
      ...
      assert.ok(!/great job/i.test(message.content));
    }
  });
  ...
  test("keeps generated customer-facing content free of monetary figures and profitability language", () => {
    for (const score of [62, 79, 90]) {
      ...
      assert.ok(!/\$\s*\d|\b\d{1,3}(?:,\d{3})+\b|\bprofit\b/i.test(`${transcript} ${coaching}`));
    }
  });
  ...
  test("backfills only empty or invalid existing transcript fields", () => {
    const plan = planSessionBackfill([
      ...
      { id: 3, transcript: buildTranscript(session(86), scenario(), 0) },
    ], 3);
    assert.deepEqual(plan, { insertSessionCount: 0, transcriptSessionIds: [1, 2] });
