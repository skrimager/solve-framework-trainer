...
// office plus Apptix's existing internal demo office. It only operates against
// the database named by DATABASE_URL; do not point it at production without
...
// These usernames already exist in Office 8. The script refuses to create a
// replacement account and only gives each existing consultant a session history.
export const APPTIX_PERSONAS: Persona[] = [
  {
    username: "demo_morgan",
    displayName: "Morgan",
    currentLevel: "beginner",
    stage: "Early beginner, first three practice attempts",
    sessions: [
      { level: "beginner", score: 61, daysAgo: 12 },
      { level: "beginner", score: 70, daysAgo: 6 },
      { level: "beginner", score: 76, daysAgo: 2 },
    ],
  },
  {
    ...
    displayName: "Taylor",
    currentLevel: "beginner",
    stage: "Mid beginner, three qualifying attempts at beginner",
    sessions: [
      { level: "beginner", score: 75, daysAgo: 24 },
      { level: "beginner", score: 81, daysAgo: 20 },
      { level: "beginner", score: 86, daysAgo: 17 },
      { level: "beginner", score: 89, daysAgo: 11 },
      { level: "beginner", score: 91, daysAgo: 5 },
    ],
  },
  {
    username: "demo_avery",
    displayName: "Avery",
    currentLevel: "intermediate",
    stage: "Just advanced to intermediate, 1 of 5 at the new level",
    sessions: [
      { level: "beginner", score: 86, daysAgo: 42 },
      { level: "beginner", score: 89, daysAgo: 38 },
      { level: "beginner", score: 87, daysAgo: 34 },
      { level: "beginner", score: 91, daysAgo: 30 },
      { level: "beginner", score: 88, daysAgo: 26 },
      { level: "intermediate", score: 78, daysAgo: 13 },
      { level: "intermediate", score: 83, daysAgo: 10 },
      { level: "intermediate", score: 87, daysAgo: 4 },
      { level: "intermediate", score: 76, daysAgo: 1 },
    ],
  },
  {
    username: "demo_riley",
    displayName: "Riley",
    currentLevel: "advanced",
    stage: "Advanced, near certification, 4 of 5 qualifying attempts at advanced",
    sessions: [
      { level: "beginner", score: 88, daysAgo: 58 },
      { level: "beginner", score: 86, daysAgo: 55 },
      { level: "beginner", score: 91, daysAgo: 52 },
      { level: "beginner", score: 87, daysAgo: 48 },
      { level: "beginner", score: 90, daysAgo: 45 },
      { level: "intermediate", score: 89, daysAgo: 41 },
      { level: "intermediate", score: 86, daysAgo: 37 },
      { level: "intermediate", score: 92, daysAgo: 34 },
      { level: "intermediate", score: 85, daysAgo: 30 },
      { level: "intermediate", score: 90, daysAgo: 27 },
      { level: "advanced", score: 87, daysAgo: 22 },
      { level: "advanced", score: 91, daysAgo: 17 },
      { level: "advanced", score: 89, daysAgo: 12 },
      { level: "advanced", score: 82, daysAgo: 10 },
      { level: "advanced", score: 84, daysAgo: 8 },
      { level: "advanced", score: 80, daysAgo: 7 },
      { level: "advanced", score: 83, daysAgo: 5 },
      { level: "advanced", score: 77, daysAgo: 4 },
      { level: "advanced", score: 88, daysAgo: 3 },
    ],
  },
];
...
function openingFor(scenario: Scenario): string {
  ...
  return cleanGeneratedText(labeled?.[1] ?? quoted?.[1] ?? `I want to talk about ${scenario.title}.`);
}
...
export function buildTranscript(session: Pick<Session, "score" | "completedAt">, scenario: Scenario, seed: number): string {
  ...
  return JSON.stringify(turnsWithTimestamps([
    ...
    ["consultant", "Good. I will send the focused next-step outline today, and we will use the follow-up to confirm the direction together."],
  ], session.completedAt));
}
...
async function backfillPersona(
  ...
  if (!user) {
    if (!allowUserCreate) {
      throw new Error(`Expected existing Apptix account ${persona.username} was not found. Refusing to create it.`);
    }
  ...
  if (plan.insertSessionCount > 0) {
    ...
    for (let seed = 0; seed < persona.sessions.length; seed++) {
      ...
      draft.transcript = buildTranscript(draft, scenario, seed);
...
async function main() {
  ...
  const apptixOffice = await storage.getOffice(APPTIX_DEMO_OFFICE_ID);
  if (!apptixOffice) {
    throw new Error(`Apptix demo office #${APPTIX_DEMO_OFFICE_ID} not found. Refusing to guess its invite code.`);
  }
  console.log(`Using Apptix office #${apptixOffice.id} (${apptixOffice.name}), invite code ${apptixOffice.inviteCode}.`);
  ...
  for (const persona of APPTIX_PERSONAS) {
    const result = await backfillPersona(persona, apptixOffice.id, false, pools, scenariosById);
    totals.usersCreated += result.usersCreated;
    totals.sessionsCreated += result.sessionsCreated;
    totals.transcriptsUpdated += result.transcriptsUpdated;
    totals.coachingCreated += result.coachingCreated;
